...
const Skills = () => {
  const [animatedLevels, setAnimatedLevels] = useState(
    skills.map(() => `${Math.floor(Math.random() * 100)}%`) // random widths
  );

  useEffect(() => {
    // Animate to real skill levels after 1.5s
            ...
            {skills.map((skill, index) => (
              <div key={skill.name}>
                ...
                <div className="w-full bg-gray-200 rounded-full h-1.5">
                  <div
                    className="bg-red-600 h-1.5 rounded-full transition-all duration-700 ease-in-out"
